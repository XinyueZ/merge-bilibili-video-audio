...
class AudioVideoMerger:
    def __init__(self, directory, res):
    ...
    def merge(self):
        for root, dirs, files in os.walk(self.directory):
            for dir_name in dirs:
                sub_directory = os.path.join(root, dir_name, str(self.res))
...
def main():
    parser = argparse.ArgumentParser(
        description="Merge audio and video files using ffmpeg."
    )
    parser.add_argument("--dir", required=True, help="Path to the main directory")
    parser.add_argument(
        "--res", required=True, type=int, help="Resolution directory name (e.g., 80)"
    )
    args = parser.parse_args()

    merger = AudioVideoMerger(args.dir, args.res)
    merger.merge()
